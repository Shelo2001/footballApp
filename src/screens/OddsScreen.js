...
import { useGetOddsQuery } from "../services/OddsApi"

const OddsScreen = () => {
  const { data, isFetching } = useGetOddsQuery()
  console.log(data)
  return (
        ...
        <table class="text-left w-full mx-auto">
          <thead class="bg-black flex mb-4 text-white w-full">
            <tr class="flex w-full mb-4 ">
              ...
              <th class="p-4 w-1/4">
                <p className="text-white font-bold text-lg">Win(Home)</p>
              </th>

              <th class="p-4 w-1/4">
                <p className="text-white font-bold text-lg">Win(Away)</p>
              </th>
              <th class="p-4 w-1/4">
          ...
          >
            {data?.map((game) => {
              return (
                <tr
                  key={game.id}
                  class="flex w-full mb-4 border-y-2 rounded min-h-fit	 border-gray-400"
                >
                  <td class="p-4 w-1/4">
                  ...
                  <td class="p-4 w-1/4 ">
                    <p className="font-semibold ">{game.sport_title}</p>
                  </td>
